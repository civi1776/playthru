...
export function computeTimePar(yardage, par, playerCount, transport, difficulty = 'intermediate', coefficients = DEFAULT_CLOCK_COEFFICIENTS, defaultYardsByPar = DEFAULT_YARDS_BY_PAR) {
...
export function scoreHole(players, elapsedSeconds, holeData, config) {
  const { scoringScale, clockCoefficients, penaltyParams, transport, playerCount, difficulty } = config;

  const par = holeData?.par;
  const rawYardage = holeData?.yardage ?? null;

  const { yards, yardageSource } = resolveYardage(rawYardage, par);
  const timeParSec = computeTimePar(rawYardage, par, playerCount, transport, difficulty, clockCoefficients);
