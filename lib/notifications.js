...
Notifications.setNotificationHandler({
  handleNotification: async (notification) => {
    // Never show live_round notifications as banners — lock screen only
    if (notification.request.content.data?.type === 'live_round') {
...
// ─── Weekly Clocked Digest — every Monday 8am ───────────────────────────────
export async function scheduleWeeklyDigest(popScore, rank, roundsThisMonth) {
  try {
    // Cancel only the previous digest + monthly challenge — NOT all notifications
    const all = await Notifications.getAllScheduledNotificationsAsync();
    const toCancel = all.filter(n => n.content.data?.type === 'weekly_digest' || n.content.data?.type === 'monthly_challenge');
    ...
    const messages = [
      { title: 'Your weekly Clocked digest', body: scoreStr ? `Your Clocked Score is ${scoreStr}. You're ranked #${rank} nationally.` : `You're ranked #${rank} nationally.` },
      { title: 'How was your week?', body: `You have ${roundsThisMonth} ${roundsThisMonth === 1 ? 'round' : 'rounds'} this month. Play another to improve your rank.` },
    ...
    await Notifications.scheduleNotificationAsync({
      content: {
        ...
        body: 'The leaderboard resets soon. Play your rounds now to lock in your rank!',
        sound: true,
