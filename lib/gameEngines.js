...
// ─── Shared hole-comparison kernel ────────────────────────────────────────────
// Counts hole wins/losses between two players over [fromIdx, toIdx) (0-indexed, toIdx exclusive).
// Returns { status: signed_int, winner: 0|1|null }
//   status > 0  → P0 up by that many holes
//   status < 0  → P0 down
//   status = 0  → all square
export function holeMatchStatus(p0, p1, fromIdx, toIdx) {
  let s = 0;
  for (let h = fromIdx; h < toIdx; h++) {
    const a = p0?.[h];
    const b = p1?.[h];
    if (a == null || b == null) continue;
    if (a < b) s++;       // P0 wins hole
    else if (b < a) s--;  // P1 wins hole
    // equal = halved, s unchanged
  }
  return { status: s, winner: s > 0 ? 0 : s < 0 ? 1 : null };
}

// ─── Nassau ───────────────────────────────────────────────────────────────────
// Settled hole-by-hole (match-play style), NOT by total strokes.
// Each hole: lower gross score wins the hole; ties are halved.
// Segment status = (holes won by P0) − (holes won by P1), signed from P0's perspective.
//
// Return shape per segment: { status: number, winner: 0|1|null }
// back is null when fewer than 9 holes have been completed (back 9 not started).
//
// NOTE: currently 2-player only (P0 vs P1). Multi-player Nassau (3+) is an
// open design decision — extra players in playerScores are ignored here.
export function calcNassau(playerScores, pars) {
  const p0 = playerScores[0];
  const p1 = playerScores[1];
  const total = pars.length;
  return {
    front: holeMatchStatus(p0, p1, 0, Math.min(9, total)),
    back:  total >= 9 ? holeMatchStatus(p0, p1, 9, Math.min(18, total)) : null,
    total: holeMatchStatus(p0, p1, 0, total),
  };
}

// ─── Match Play ───────────────────────────────────────────────────────────────
export function calcMatchPlay(playerScores, pars, players) {
  ...
  for (let h = 0; h < pars.length; h++) {
    ...
    if (s0 < s1)      { status++; holeResults.push(0); }
    else if (s1 < s0) { status--; holeResults.push(1); }
...
// ─── Stableford (retired side game — NOT the Clocked Scoring system) ─────────
export function calcStableford(playerScores, pars) {
...
// ─── Press scoring (Nassau only) ─────────────────────────────────────────────
// press: { startHole, endHole } — both 1-indexed, inclusive.
// Returns { status, winner } same shape as holeMatchStatus.
export function calcPressStatus(press, p0Scores, p1Scores) {
  const fromIdx = press.startHole - 1;
  const toIdx   = press.endHole;      // toIdx is exclusive === endHole (1-indexed last hole)
  return holeMatchStatus(p0Scores, p1Scores, fromIdx, toIdx);
}

// ─── Dispatcher ───────────────────────────────────────────────────────────────
export function calcGame(type, playerScores, pars, players) {
  switch (type) {
    case 'Skins':             return calcSkins(playerScores, pars);
    case 'Nassau':            return calcNassau(playerScores, pars);
    case 'Match Play':        return calcMatchPlay(playerScores, pars, players);
    case 'Stableford':        return calcStableford(playerScores, pars);
...
export function gameResultToUnitScores(type, result, n) {
  switch (type) {
    ...
    case 'Nassau': {
      ...
      if (result.front?.winner != null) s[result.front.winner] += 1;
      if (result.back?.winner  != null) s[result.back.winner]  += 1;
      if (result.total?.winner != null) s[result.total.winner] += 1;
      return s;
    }
    case 'Match Play': {
      const s = Array(n).fill(0);
