...
// timePar = overhead + (yards × travelRate) + (players − 1) × playerAdj
//
// Yardage-driven (par is not used in the formula but still needed for
// resolveYardage fallback when course data lacks hole yardage).
// Always returns a positive integer (seconds).
...
export function computeTimePar(yardage, par, playerCount, transport, coefficients = DEFAULT_CLOCK_COEFFICIENTS, defaultYardsByPar = DEFAULT_YARDS_BY_PAR) {
  ...
  return Math.round(
    ...
    (yards * travelRate) +
    (Math.max(0, playerCount - 1) * coefficients.player_adj_seconds)
  );
}
